...
if (isset($_GET["kategori_seourl"])) {
	$kategorisor = $db->prepare("SELECT * FROM kategori where kategori_seourl=:kategori_seourl");
	$kategorisor->execute(
		array(
			'kategori_seourl' => $_GET["kategori_seourl"]
		)
	);
	$kategoricek = $kategorisor->fetch(PDO::FETCH_ASSOC);
	$kategori_seourl = $kategoricek['kategori_seourl'];
	$blogsor = $db->prepare("SELECT * FROM blog where category_seourl=:category_seourl and blog_status=:blog_status  ORDER BY blog_id DESC");
	$blogsor->execute(array(
		'category_seourl' => $kategori_seourl,
		'blog_status' => 1
	));
} else {
	$blogsor = $db->prepare("SELECT * FROM blog where blog_status=:blog_status ORDER BY blog_id DESC");
	$blogsor->execute(array(
		'blog_status' => 1
	));
}
...
<section class="page-header">
	<div class="container-xl">
		<div class="text-center">
			<h1 class="mt-0 mb-2">Kategoriler</h1>
